...
const studentList = [
    ...
    {
        name: "Инокентьев Инокентий Инокентьевич",
        date: "1999-05-05",
        year: "2017-01-01",
...
function createTableApp(container, table) {
...
function drawStudendFromList() {
    for (const obj of studentList) {
        let name = obj.name;
        let date = obj.date;
        let year = obj.year;
...
function sortTableByColumn(tableBody, column, ascending = true) {
    ...
    tableHead
        .querySelectorAll("th")
        .forEach((th) => th.classList.remove("ascending-th", "descending-th"));
    tableHead
        .querySelector(`th:nth-child(${column + 1})`)
        .classList.toggle("ascending-th", ascending);
    tableHead
        .querySelector(`th:nth-child(${column + 1})`)
        .classList.toggle("descending-th", !ascending);
}
    ...
    document.querySelectorAll(".table th").forEach((header) => {
        header.addEventListener("click", () => {
            ...
            const currentlyAscending =
                header.classList.contains("ascending-th");
    ...
    inputFullName,
        ...
        inputFaculty.addEventListener("input", () => {
            ...
            if (
                ...
                alert(
                    `Дата рождения не может быть меньше ${inputDate.min} и больше ${inputDate.max}`
                );
...
function focusInput(
    ...
    inputYearOfStudy.addEventListener("keydown", function (e) {
        if (e.code === "Enter") {
            if (
                inputYearOfStudy.value < inputYearOfStudy.min ||
                inputYearOfStudy.value > inputYearOfStudy.max
            ) {
                alert(
                    `Дата поступления в университет не может быть меньше ${inputYearOfStudy.min} и больше ${inputYearOfStudy.max}`
                );
